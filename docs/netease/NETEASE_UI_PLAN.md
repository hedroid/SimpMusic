...
- **短期:写路径收口 + 对账补齐**(一次会话量级)
  1. C 补齐:网易 getLibraryPlaylists 与 YT split 拉到云端列表后,对本地 playlist 表
     liked=1 且不在云端集合的行清零(照抄关注分区模式:拉取完整才清,失败不动本地);
     红心歌单计数不在此列(计数非布尔,对账语义不同)。
  2. B 收口:盘点全部云端写点(关注/红心/收藏/歌单增删/移除歌曲),确认每个成功后都有
     "回写镜像+LibraryMutationBus 事件";缺的补上(现有:歌单四写+艺人取关;待查:批量红心、
     下载触发点的镜像)。
  3. 验收:YTM/网易网页端改关注/收藏后,app 进对应分区一轮拉取内自动对齐;弱网快速返回
     不丢事件(NonCancellable+pending 已具备)。
- **中期:id 规范化专项**(一轮专项)
  列表 API 返回别名 id、页面 API 返回 canonical(陈奕迅实锤)。策略:懒式规范化——艺人页
  写行已用 canonical(65bed42);对账清除时对"疑似别名行"(不在云端集合但该艺人近期被浏览
  过)跳过或二次确认,防止误清 canonical 行。彻底方案(写入侧全量换 canonical,每艺人多一次
  请求)成本高,除非别名问题再现否则不做。
- **长期:状态字段退役(E,随迭代分摊)**
