...
- **行缓存**：每行独立 10min 会话缓存（`RowCache`，命中即返回、miss 才网络并回写；
  ...
  仅可见行立即 force 重拉。
- **下拉刷新竞态修复（2026-09-20）**：曾报"顶部转圈好多次、一直刷新的感觉"——根因是
  `ensureRowLoaded` 完成回写用"读 `_state.value`→拼行→整体赋值"的非原子读改写：刷新后多行
  并行完成撞车，后写者把先写者的 Ready 覆盖回 Loading，该行占位重组后**再次拉取再次转圈**，
  多行竞态级联。修法=全部回写改 `MutableStateFlow.update{}`（原子 CAS，`retryRow`/
  `loadNewAlbumsArea` 同步换掉）；次因 `forceNextLoads` 置位后永不复位（注释语义是"本次"），
  现在全部行离开 Loading 即复位——否则刷新一次之后所有后续行加载都绕过行缓存打网络。
  实测：下拉→五行占位→1s 内全部 Ready→逐帧 3s 无回圈。
