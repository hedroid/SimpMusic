...
- **状态同步 + 收藏管理二轮（2026-09-15，159c3491 + core adb78fd）**：
  ...
  - **歌单收藏同步云村（2026-09-15 三轮）**：任何入口打开网易歌单点红心收藏，云端同步
    `/playlist/subscribe`（取消收藏=t=0）。**与艺人关注共用"关注与网易云同步"开关**（用户
    定案：收藏歌单=订阅，与 /artist/sub 同属 subscribe 语义，不单设开关；关注开关描述已
    扩到涵盖歌单）。自建歌单跳过云端（自己的歌单无"收藏"概念）；本地 Room 标记总是生效。
    注意 /playlist/subscribe 有端点级 405 频控（见 core PITFALLS），脚本连发会触发。
