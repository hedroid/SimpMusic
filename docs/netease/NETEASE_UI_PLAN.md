...
- **下拉刷新=静默刷新（2026-09-20 终稿，三轮迭代）**：已就绪的行**保持内容不回占位**，
  后台 force 重拉（绕过行缓存）、落地即原位替换；失败行回 Loading 重试；未加载行维持原状。
  顶部指示器（M3 PullToRefresh 标准）**只陪跑到第一批重拉落地**即收起——各行并行重拉，
  最慢的行（分类目录/排行榜多接口聚合）能到 10s+，指示器等全部行就是"顶端转圈转好多圈"；
  收起后其余行继续静默原位替换。初版是"全部行重置 Loading 占位、各行自行转圈"（观感"一直
  在刷新"）；一轮修了行回写丢更新竞态（先读后写→`MutableStateFlow.update{}`）和
  `forceNextLoads` 永不复位；二轮改静默刷新但指示器仍等全部行；三轮指示器改首批落地收。
  `forceNextLoads` 已删除（refresh 对已加载行显式 force，滚动加载永远走缓存）。
  实测：9 行全加载下拉,指示器 ~2s 收起（修复前 14s+），行内容全程不动。
